...
//2.0.9 version was remade to be compatible with 90% of payments gates and any another woocommerce plugins!!!
//22-07-2015
final class WOOCS
{

    //http://docs.woothemes.com/wc-apidocs/class-WC_Order.html
    public $the_plugin_version = '1.1.1';
    ...
    public function wp_head()
    {
        ...
        //+++
        //trick for refreshing header cart after currency changing
    ...
    public function woocommerce_get_price_html($price_html)
    {
        ...
        if ($this->current_currency == 'JPY' OR $hide_cents == 1)
        {
            if ($this->decimal_sep == '.')
            {
...
/*
        //$_REQUEST['woocs_woocommerce_before_mini_cart'] - exists only when mini cart refreshing
        if (isset($_REQUEST['action']) AND $_REQUEST['action'] == 'woocommerce_get_refreshed_fragments')
        {
            if (isset($_REQUEST['woocs_woocommerce_before_mini_cart'])
                    AND $_REQUEST['woocs_woocommerce_before_mini_cart'] == 'mini_cart_refreshing')
            {
                return $price;
            }
        }
*/
    ...
    public function get_rate()
    {
        ...
        {
            //stupid trick for that who believes in P3, because get_rate works only by cron
            return;
        }
        ...
        switch ($mode)
        {
            case 'yahoo':
                /*
                  $url = 'http://finance.yahoo.com/d/quotes.txt?e=.txt&f=c4l1&s=' . $this->default_currency . $_REQUEST['currency_name'] . '=X';
    ...
    {
        //just for not count twice woocommerce_order_amount_total
